...
import { type CreateMenuVariantInput, createMenuVariantSchema } from '../schemas'
import type { ProductType } from './MenuCards'
...
export default function AdminMenuDashboard({
  ...
  const deleteMutation = useMutation({
    mutationFn: async ({ id, name }: { id: string; name: string }) => {
      const res = await api<{ success?: boolean; error?: string }>(`/api/admin/menu/${id}`, {
        method: 'DELETE'
      })
    ...
    onError: (error: FetchError | Error) => {
      const msg =
        error instanceof FetchError ? error.data?.error || 'Gagal menghapus menu' : error.message
  ...
  const saveMutation = useMutation({
    mutationFn: async (validData: any) => {
      ...
      const res = await api<{ success?: boolean; data: any; error?: string; message?: string }>(
        url,
        {
          method: method,
          body: validData
      ...
      if (res.error || (!res.success && res.message))
        throw new Error(res.error || res.message || 'Terjadi kesalahan')
    ...
    onError: (error: FetchError | Error) => {
      const msg =
        error instanceof FetchError
          ? error.data?.error || error.data?.message || 'Terjadi kesalahan'
          : error.message
      toast.error(msg)
    }
  })
    ...
    return (
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-xl max-w-xl mx-auto border-t-8 border-brown-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, wholesaleKembung: Number(e.target.value) })
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, wholesaleLumpia: Number(e.target.value) })
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, wholesaleKrispy: Number(e.target.value) })
                }
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-brown-500 uppercase tracking-wider mb-2">
              Deskripsi Topping (Penting untuk Penjualan)
            </label>
            <input
              type="text"
          ...
          <div>
            ...
            <p className="text-[10px] text-brown-400 mt-2">
              Pilih tag agar badge muncul di kartu produk pelanggan. Jangan ada typo!
            </p>
          </div>
      ...
      <div className="p-6 flex justify-between items-center bg-brown-700 border-b border-brown-800 text-cream-100">
        <div>
          ...
          <p className="text-xs text-cream-200/80 mt-1">
            Kelola varian rasa, harga, dan gambar menu yang tampil di halaman utama
          </p>
        </div>
        <button
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-cream-100/50 text-brown-800 border-b border-cream-200">
              <th className="p-4 text-xs font-bold uppercase tracking-wider text-center w-16">
                No.
              </th>
              ...
              <th className="p-4 text-xs font-bold uppercase tracking-wider text-center w-28">
                Stok
              </th>
              <th className="p-4 text-xs font-bold uppercase tracking-wider text-center w-48">
                Aksi
              </th>
            </tr>
          </thead>
          <tbody>
            {products.map((p, index) => (
              ...
              >
                ...
                <td className="p-4">
                  <div className="font-serif font-bold text-brown-800 text-base">
                    {p.flavorName}
                  </div>
                </td>
                <td className="p-4 font-bold text-brown-700 text-base">
                  <span className="text-xs font-normal">K:</span> Rp{' '}
                  {p.priceKembung.toLocaleString('id-ID')}
                  <br />
                  <span className="text-xs font-normal">L:</span> Rp{' '}
                  {p.priceLumpia.toLocaleString('id-ID')}
                  <br />
                  <span className="text-xs font-normal">Kr:</span> Rp{' '}
                  {p.priceKrispy.toLocaleString('id-ID')}
                </td>
                <td className="p-4">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${p.stock > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                    >
