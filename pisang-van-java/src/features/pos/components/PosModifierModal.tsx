...
interface PosModifierModalProps {
  ...
  toppings: Topping[]
  onClose: () => void
  onAdd: (orderItem: { product: ProductType, baseType: string, topping: Topping | null, quantity: number, subtotal: number }) => void
}

export default function PosModifierModal({ product, toppings, onClose, onAdd }: PosModifierModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-red-500 font-bold p-2 text-2xl leading-none">&times;</button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="mb-8">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">1. Pilih Jenis (Wajib)</h3>
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all active:scale-95 ${
                      isSelected ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-gray-200 hover:border-orange-200 text-gray-600'
                    }`}
                  >
          ...
          <div className="mb-8">
            <div className="flex justify-between items-end mb-3">
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider">2. Topping (Opsional)</h3>
              {selectedTopping && (
                <button onClick={() => setSelectedTopping(null)} className="text-xs text-red-500 font-bold">Hapus Topping</button>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {toppings.filter(t => t.isActive).map(topping => {
                ...
                return (
                  ...
                  >
                    <div>
                      <div className="font-bold text-gray-800">{topping.emoji} {topping.name}</div>
          ...
          {/* STEP 3: Quantity */}
          <div>
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">3. Jumlah</h3>
