...
import PosModifierModal, { Topping } from './PosModifierModal'
          ...
          return (
            <button
              ...
              className={`relative flex flex-col h-32 rounded-2xl overflow-hidden shadow-sm border transition-all active:scale-95 ${
                available ? 'border-gray-200 hover:border-orange-400 bg-white' : 'border-gray-200 bg-gray-100 opacity-60 cursor-not-allowed'
              }`}
            >
              {/* Product Background Image / Fallback Color */}
              <div className="absolute inset-0 bg-gray-100">
                {product.imageUrl && (
                  <Image src={product.imageUrl} alt={product.flavorName} fill className="object-cover opacity-30" />
                )}
              ...
              <div className="relative z-10 flex flex-col justify-end p-3 h-full text-left">
                <h3 className="font-bold text-gray-800 text-lg leading-tight line-clamp-2">{product.flavorName}</h3>
                {available ? (
                  <span className="text-xs text-green-600 font-bold mt-1">Stok: {product.stock}</span>
                ) : (
