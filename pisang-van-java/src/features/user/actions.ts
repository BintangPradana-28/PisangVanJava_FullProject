...
const deleteAccountSchema = z
  .object({
    confirmationString: z.string().refine((val) => val === 'HAPUS AKUN SAYA', {
      message: "String konfirmasi harus persis 'HAPUS AKUN SAYA'"
    })
  })
  .strict()
...
export async function deleteAccountPermanently(formData: FormData) {
  try {
    ...
    return {
      success: true,
      message: 'Akun Anda beserta seluruh data sensitif telah dihapus secara permanen.'
    }
  } catch (error) {
    ...
    return {
      success: false,
      error: 'Terjadi kesalahan sistem saat menghapus akun. Silakan hubungi dukungan teknis.'
    }
  }
}
