...
function generateTestSignature(overrides?: Partial<typeof TEST_PARAMS>): string {
  ...
  return createHash('sha512')
    .update(`${params.orderId}${params.statusCode}${params.grossAmount}${TEST_SERVER_KEY}`)
    .digest('hex')
}
...
describe('mapMidtransStatusToPaymentStatus', () => {
  describe('settlement (QRIS, GoPay, bank transfer)', () => {
    it('settlement → PAID', () => {
      expect(mapMidtransStatusToPaymentStatus('settlement')).toBe(PaymentStatus.PAID)
    })

    it('settlement tidak peduli fraud_status (tidak relevan)', () => {
      // fraud_status hanya relevan untuk credit_card capture
      expect(mapMidtransStatusToPaymentStatus('settlement', 'challenge')).toBe(PaymentStatus.PAID)
    })
  })

  describe('capture (credit card)', () => {
    it('capture + fraud accept → PAID', () => {
      expect(mapMidtransStatusToPaymentStatus('capture', 'accept')).toBe(PaymentStatus.PAID)
    })

    it('capture + fraud challenge → CHALLENGE (butuh review manual)', () => {
      expect(mapMidtransStatusToPaymentStatus('capture', 'challenge')).toBe(PaymentStatus.CHALLENGE)
    })

    it('capture tanpa fraud_status → PAID (default)', () => {
      expect(mapMidtransStatusToPaymentStatus('capture')).toBe(PaymentStatus.PAID)
    })
  })

  describe('status lainnya', () => {
    it('pending → PENDING', () => {
      expect(mapMidtransStatusToPaymentStatus('pending')).toBe(PaymentStatus.PENDING)
    })

    it('deny → FAILED', () => {
      expect(mapMidtransStatusToPaymentStatus('deny')).toBe(PaymentStatus.FAILED)
    })

    it('cancel → CANCELED', () => {
      expect(mapMidtransStatusToPaymentStatus('cancel')).toBe(PaymentStatus.CANCELED)
    })

    it('expire → EXPIRED', () => {
      expect(mapMidtransStatusToPaymentStatus('expire')).toBe(PaymentStatus.EXPIRED)
    })

    it('refund → REFUNDED', () => {
      expect(mapMidtransStatusToPaymentStatus('refund')).toBe(PaymentStatus.REFUNDED)
    })

    it('partial_refund → REFUNDED', () => {
      expect(mapMidtransStatusToPaymentStatus('partial_refund')).toBe(PaymentStatus.REFUNDED)
    })

    it('authorize → PENDING (pre-auth belum captured)', () => {
      expect(mapMidtransStatusToPaymentStatus('authorize')).toBe(PaymentStatus.PENDING)
    })
  })
})
