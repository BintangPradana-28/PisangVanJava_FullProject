...
import { extractClientIp, isMidtransIp, isRequestViaCloudflare } from '@/src/lib/ip-utils'
...
const midtransWebhookSchema = z.object({
  ...
  gross_amount: z.string().regex(/^\d{1,10}(\.\d{2})?$/, 'gross_amount format tidak valid'),
  signature_key: z.string().length(128, 'signature_key harus 128 karakter SHA512 hex'),
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!validation.success) {
    ...
    const orderId =
      typeof rawBody === 'object' && rawBody !== null && 'order_id' in rawBody
  ...
  if (!serverKey) {
    console.error('[Midtrans Webhook] MIDTRANS_SERVER_KEY tidak terkonfigurasi')
  ...
  if (!isSignatureValid) {
    ...
    return NextResponse.json({ message: 'Signature tidak valid' }, { status: 400 })
  }
  ...
  try {
    ...
    if (error instanceof PaymentNotFoundError) {
      ...
      return NextResponse.json({ message: 'Payment record tidak ditemukan' }, { status: 404 })
    }
