...
export class StockManager {
  ...
  static async reserve(
    variantId: string,
    quantity: number
  ): Promise<'SUCCESS' | 'INSUFFICIENT_STOCK' | 'CACHE_MISS'> {
    try {
      if (quantity <= 0) throw new Error('Quantity must be greater than zero.')

      const key = StockManager.getKey(variantId)
  ...
  static async rollback(variantId: string, quantity: number): Promise<void> {
    try {
      ...
      console.error(
        '[StockManager] CRITICAL: Failed to rollback stock for %s. Potential ghost stock!',
        variantId,
        error
      )
    }
