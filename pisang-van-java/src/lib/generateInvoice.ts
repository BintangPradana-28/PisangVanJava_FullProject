...
export const generateInvoicePDF = (order: any) => {
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(
    `Subtotal Produk: Rp ${new Intl.NumberFormat('id-ID').format(order.totalPrice - order.deliveryFee + order.discountAmount)}`,
    130,
    finalY
  )
  doc.text(
    `Ongkos Kirim: Rp ${new Intl.NumberFormat('id-ID').format(order.deliveryFee)}`,
    130,
    finalY + 6
  )
  ...
  if (order.discountAmount > 0) {
    ...
    doc.text(
      `Diskon Voucher: -Rp ${new Intl.NumberFormat('id-ID').format(order.discountAmount)}`,
      130,
      finalY + 12
    )
    doc.setTextColor(40)
  ...
  doc.text(
    `TOTAL BAYAR: Rp ${new Intl.NumberFormat('id-ID').format(order.totalPrice)}`,
    130,
    totalY
  )
