...
export async function POST(req: Request) {
  try {
    ...
    const variantIds = [...new Set(items.map((i: any) => i.menuVariantId).filter(Boolean))] as string[];
    const toppingIds = [...new Set(items.flatMap((i: any) => {
    ...
    const [variants, toppings] = await Promise.all([
      prisma.menuVariant.findMany({
        ...
        select: { id: true, priceKembung: true, priceLumpia: true, priceKrispy: true, flavorName: true }
      }),
    ...
    const validatedItems = items.map((item: any) => {
      ...
      const rawToppings = Array.isArray(item.toppings) ? item.toppings : (item.topping ? [item.topping] : []);
      const trueToppings = rawToppings.map((t: any) => {
