...
export async function GET() {
  ...
  const [allOrders, byStatus, recentOrders] = await Promise.all([
    prisma.order.findMany({
      ...
      select: { totalPrice: true, status: true, createdAt: true, source: true }
    }),
  ...
  const totalRevenue = allOrders
    .filter((o: any) => o.status === 'done')
    .reduce((s: any, o: any) => s + o.totalPrice, 0)
  const totalOrders = allOrders.length
  const doneOrders = allOrders.filter((o: any) => o.status === 'done').length
  const sourceCount = allOrders.reduce((acc: Record<string, number>, o: any) => {
