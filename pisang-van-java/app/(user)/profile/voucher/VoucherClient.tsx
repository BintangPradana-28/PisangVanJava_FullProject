...
import { Ticket, Coins, Clock, ChevronRight } from 'lucide-react'
...
export default function VoucherClient({ koinPisang, vouchers }: { koinPisang: number, vouchers: VoucherData[] }) {
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <section className="bg-gradient-to-br from-amber-400 to-amber-600 rounded-3xl p-6 md:p-8 shadow-lg shadow-amber-200 dark:shadow-none text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-white/90 text-sm font-bold uppercase tracking-wider mb-1">Koin Pisang Saya</h2>
              <p className="text-3xl md:text-4xl font-black drop-shadow-md">{formatPrice(koinPisang)}</p>
            </div>
          </div>
      ...
      <section className="bg-white dark:bg-zinc-900 rounded-3xl p-6 md:p-8 shadow-sm border border-zinc-200/50 dark:border-zinc-800/80">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold font-serif text-zinc-900 dark:text-zinc-100">Kupon Tersedia</h2>
        ...
        {vouchers.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-12 px-4 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl">
             ...
             <p className="text-zinc-500 font-medium">Belum ada kupon yang tersedia untuk Anda saat ini.</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="mb-4">
                  <h3 className="text-lg font-black text-zinc-800 dark:text-zinc-100 mb-1">
                    Diskon {v.discountType === 'PERCENTAGE' ? `${v.discountValue}%` : formatPrice(v.discountValue)}
                  </h3>
                  {v.discountType === 'PERCENTAGE' && v.maxDiscount && (
                    <p className="text-xs text-zinc-500">Maks. potongan {formatPrice(v.maxDiscount)}</p>
                  )}
                  <p className="text-xs text-zinc-500 mt-1">Min. belanja {formatPrice(v.minPurchase)}</p>
                </div>
