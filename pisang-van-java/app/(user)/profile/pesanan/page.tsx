...
import { ShoppingBag, ChevronDown, ChevronUp, Package, Clock, CheckCircle2, XCircle, AlertCircle, RefreshCw, CreditCard, Download, Repeat } from 'lucide-react'
...
const getStatusConfig = (status: string) => {
  switch (status) {
    case 'PENDING_PAYMENT':
      return { color: 'bg-yellow-100 text-yellow-700 border-yellow-200', icon: Clock, label: 'Menunggu Pembayaran' }
    case 'PROCESSING':
      return { color: 'bg-blue-100 text-blue-700 border-blue-200', icon: RefreshCw, label: 'Sedang Diproses' }
    case 'READY':
      return { color: 'bg-purple-100 text-purple-700 border-purple-200', icon: Package, label: 'Siap Diambil/Dikirim' }
    case 'COMPLETED':
      return { color: 'bg-green-100 text-green-700 border-green-200', icon: CheckCircle2, label: 'Selesai' }
    case 'CANCELED':
      return { color: 'bg-red-100 text-red-700 border-red-200', icon: XCircle, label: 'Dibatalkan' }
    default:
      return { color: 'bg-zinc-100 text-zinc-700 border-zinc-200', icon: AlertCircle, label: status }
  }
}
  ...
  if (currentStatus === 'CANCELED') {
    return (
      <div className="flex items-center gap-2 p-4 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30">
        ...
        <span className="text-sm font-bold text-red-700 dark:text-red-400">Pesanan ini telah dibatalkan.</span>
      </div>
    )
  }
        ...
        {stages.map((stage, index) => {
          ...
          return (
            <div key={stage.id} className="flex flex-col items-center relative z-10 w-24">
              ...
              <span className={`text-[10px] md:text-xs font-semibold mt-3 text-center transition-colors ${
                isActive ? 'text-[#D4802A]' : isCompleted ? 'text-zinc-700 dark:text-zinc-300' : 'text-zinc-400'
              }`}>
                {stage.label}
              </span>
            </div>
      ...
      <div className="space-y-4">
        {loading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-zinc-100 dark:bg-zinc-800 animate-pulse h-32 rounded-3xl"></div>
          ))
        ) : orders.length === 0 ? (
          ...
          orders.map((order) => {
            ...
            return (
              <div key={order.id} className={`bg-white dark:bg-zinc-900 rounded-2xl border transition-all overflow-hidden ${isExpanded ? 'border-[#D4802A] ring-1 ring-[#D4802A]/20 shadow-md' : 'border-zinc-200 dark:border-zinc-800 shadow-sm'}`}>
                ...
                >
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <span className={`px-2.5 py-1 rounded-md text-xs font-bold border flex items-center gap-1.5 ${statusConfig.color}`}>
                    ...
                    <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">
                      Order ID: <span className="font-mono text-xs ml-1 bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded text-zinc-500">{order.id}</span>
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6">
                    ...
                    <div className={`transition-transform duration-300 ${isExpanded ? 'rotate-180 text-[#D4802A]' : 'text-zinc-400'}`}>
                      ...
                      <div className="p-5 md:p-7 space-y-8">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          ...
                          <div className="lg:col-span-2 space-y-4">
                            <h4 className="text-sm font-bold text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 pb-2">Rincian Produk</h4>
                            <div className="space-y-4">
                              {order.items.map((item) => (
                                <div key={item.id} className="flex items-start gap-4">
                                  <div className="w-16 h-16 rounded-xl bg-zinc-200 dark:bg-zinc-800 overflow-hidden flex-shrink-0 shadow-sm">
                                    {item.variant?.imageUrl ? (
                                      <img src={item.variant.imageUrl} alt={item.variant.flavorName} className="w-full h-full object-cover" />
                                    ) : (
                                      <div className="w-full h-full flex items-center justify-center text-zinc-400 text-xs font-bold">Menu</div>
                                    )}
                                  </div>
                                  <div className="flex-1">
                                    ...
                                    <p className="text-xs text-zinc-500 mt-1">
                                      {item.quantity}x {item.baseType} @ Rp {new Intl.NumberFormat('id-ID').format(item.unitPrice)}
                                    </p>
                                    {item.toppings && item.toppings.length > 0 && (
                                      <div className="mt-2 flex flex-wrap gap-1">
                                        {item.toppings.map(t => (
                                          <span key={t.name} className="px-2 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded text-[10px] font-medium text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">
                                            + {t.name} (Rp {new Intl.NumberFormat('id-ID').format(t.price)})
                                          </span>
                                        ))}
                                      </div>
                                    )}
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 border border-zinc-200 dark:border-zinc-800 shadow-sm">
                              ...
                              {order.payment?.status && (
                                <p className={`text-xs font-bold mt-1 ${order.payment.status === 'PAID' ? 'text-green-600' : 'text-yellow-600'}`}>
                            ...
                            <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-3">
                              <h4 className="text-sm font-bold text-zinc-800 dark:text-zinc-200 mb-2">Ringkasan Biaya</h4>
                              
                              <div className="flex justify-between text-xs text-zinc-600 dark:text-zinc-400">
                                <span>Subtotal Produk</span>
                                <span>Rp {new Intl.NumberFormat('id-ID').format(order.totalPrice - order.deliveryFee + order.discountAmount)}</span>
                              </div>
                              
                              <div className="flex justify-between text-xs text-zinc-600 dark:text-zinc-400">
                                <span>Ongkos Kirim ({order.deliveryMethod})</span>
                                <span>Rp {new Intl.NumberFormat('id-ID').format(order.deliveryFee)}</span>
                              </div>

                              {order.discountAmount > 0 && (
                                <div className="flex justify-between text-xs text-green-600 dark:text-green-400 font-medium">
                                  <span>Diskon Voucher</span>
                                  <span>-Rp {new Intl.NumberFormat('id-ID').format(order.discountAmount)}</span>
                                </div>
                              )}

                              <div className="border-t border-zinc-100 dark:border-zinc-800 pt-3 flex justify-between items-center">
                                <span className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Total Belanja</span>
                                <span className="text-base font-bold text-[#D4802A]">Rp {new Intl.NumberFormat('id-ID').format(order.totalPrice)}</span>
                              </div>
                            </div>
                            
                            {/* Payment Action Button */}
                            {order.status === 'PENDING_PAYMENT' && (
                              <Link href={`/checkout/payment?orderId=${order.id}`} className="block w-full bg-[#D4802A] hover:bg-[#b56d24] text-white px-6 py-3 rounded-xl font-bold text-sm text-center transition-all shadow-md active:scale-95 mb-3">
