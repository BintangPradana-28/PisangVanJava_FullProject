...
import { ShieldCheck, Smartphone, BellRing, Trash2 } from "lucide-react";
import { 
...
export default function SecurityClient({ initialPrefs, twoFactorEnabled: init2FA, activeSessions: initSessions, hasPassword }: Props) {
    ...
    <div className="space-y-10">
        ...
        {show2FA && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div className="bg-white p-6 rounded-2xl max-w-sm w-full shadow-2xl relative">
              <button onClick={() => setShow2FA(false)} className="absolute top-4 right-4 text-gray-400 hover:text-black">✕</button>
              <h4 className="text-lg font-bold text-brown-900 mb-2">Setup Authenticator</h4>
              <p className="text-sm text-gray-600 mb-4">Pindai kode QR ini menggunakan aplikasi Google Authenticator atau Authy.</p>
              ...
              <p className="text-xs text-center text-gray-500 font-mono mb-4 break-all">Secret: {secret}</p>
      ...
      <section className="bg-white/50 backdrop-blur-md border border-brown-900/10 p-6 rounded-2xl">
        ...
        <div className="space-y-3">
          ...
          {sessions.map((sess) => (
            <div key={sess.sessionId} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-gray-50/50 rounded-xl border border-gray-100">
              <div>
                <p className="font-medium text-sm text-brown-900 flex items-center gap-2">
                  {sess.device || "Unknown Browser"}
                  {sess.current && <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Perangkat Ini</span>}
                </p>
      ...
      <section className="bg-red-50/50 backdrop-blur-md border border-red-100 p-6 rounded-2xl">
        ...
        {!showDelete ? (
          ...
          <form onSubmit={handleDeleteAccount} className="bg-white p-4 rounded-xl border border-red-200">
            ...
            <div className="flex gap-2">
              <button type="submit" className="bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-red-700 transition">
                Ya, Hapus Permanen
              </button>
              <button type="button" onClick={() => setShowDelete(false)} className="text-gray-500 text-sm px-4 py-2 font-medium hover:bg-gray-100 rounded-lg">
                Batal
              </button>
            </div>
          </form>
