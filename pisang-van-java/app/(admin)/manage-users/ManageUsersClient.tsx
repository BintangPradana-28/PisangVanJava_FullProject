...
export default function ManageUsersClient() {
  ...
  const { data: users = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const data = await api<{ success: boolean; data: UserType[]; message?: string }>(`/api/admin/users?deleted=${filterDeleted}`)
  ...
  const roleMutation = useMutation({
    mutationFn: async ({ userId, role }: { userId: string, role: string }) => {
      ...
      if (!data.success && (data as any).error) throw new Error((data as any).error || 'Gagal mengubah peran')
      return data
    },
    onSuccess: (data) => {
    ...
    onError: (error: FetchError | Error) => {
      const msg = error instanceof FetchError ? (error.data?.message || 'Gagal mengubah peran') : error.message
      toast.error(msg)
    }
  })
  ...
  const coinMutation = useMutation({
    mutationFn: async () => {
      ...
      const data = await api<{ success: boolean; error?: string; newBalance?: number }>('/api/admin/users/coins', {
        method: 'POST',
    ...
    onError: (error: FetchError | Error) => {
      const msg = error instanceof FetchError ? (error.data?.error || 'Gagal mengubah koin') : error.message
      toast.error(msg)
    }
  })
      ...
      <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-cream-200">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse whitespace-nowrap">
              ...
              <tr key={user.id} className="hover:bg-cream-50 transition-colors">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-orange-600">{user.koinPisang.toLocaleString('id-ID')}</span>
                    <button 
                ...
                <td className="p-4 text-xs">
                  <p><span className="text-gray-500">Kode:</span> {user.referralCode || '-'}</p>
                  <p><span className="text-gray-500">Dari:</span> {user.referredBy || '-'}</p>
                </td>
            ...
            {users.length === 0 && (
              <tr>
                <td colSpan={4} className="p-8 text-center text-gray-500">Belum ada pengguna di kategori ini</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleAdjustCoin} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Jumlah (Gunakan minus untuk mengurangi)</label>
                <input 
                  type="number" 
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Alasan (Untuk Audit)</label>
                <input 
                  type="text" 
                  value={coinReason}
