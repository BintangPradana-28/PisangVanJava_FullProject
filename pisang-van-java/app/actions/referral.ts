...
export async function applyReferralCode(formData: FormData) {
  ...
  if (currentUser.hasOrdered)
    throw new Error('Kode rujukan hanya untuk pengguna yang belum pernah memesan.')
  if (currentUser.referralCode === code)
    throw new Error('Tidak dapat memasukkan kode rujukan milik sendiri.')
